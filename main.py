...
def index_submitted():
    ...
    if len(username) > 20 or len(username) < 3:
        username_error = 'Invalid username. It needs to be between 3-20 characters. Try again.'
    ...
    if len(password) > 20 or len(password) < 3:
        password_error = 'Come on now...the length must be between 3-20 characters. Try again.'
    else:
        password_check = True

    #Verify Password
    if password != verify_password:
        verify_error = "Passwords don't match. Get it together."
    else:
        verify_check = True
